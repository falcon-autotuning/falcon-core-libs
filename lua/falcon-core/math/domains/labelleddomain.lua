...
function LabelledDomain.new(domain, name, pseudo_name, itype, units, desc)
    ...
    local s_pseudo = (type(pseudo_name) == "string") and require("falcon-core.instrument_interfaces.connection").new(pseudo_name) or pseudo_name
    local s_itype = lib.String_wrap(itype or "")
    local s_units = (type(units) == "string") and require("falcon-core.math.symbolunit").from_string(units) or units
