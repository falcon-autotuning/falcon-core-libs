...
class TestConfig:
    ...
    def test_ctor_from_json(self):
        try:
            Config.from_json("test_string")
        except Exception as e:
            print(f'Constructor from_json failed: {e}')

    def test_ctor_new(self):
        try:
            Config.new(Connections.new_empty(), Connections.new_empty(), Connections.new_empty(), Connections.new_empty(), Connections.new_empty(), Map[Gname, Group](), Impedances.new_empty(), VoltageConstraints.new(Adjacency.new(array.array('i', [1]), array.array('L', [1, 1]), 2, Connections.from_list([Connection.new_plunger('P1')])), 0.0, Pair[float, float](0.0, 0.0)))
        except Exception as e:
