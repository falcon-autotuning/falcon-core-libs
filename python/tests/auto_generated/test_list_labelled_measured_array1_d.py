...
class TestListLabelledMeasuredArray1D:
    def setup_method(self):
        self.obj = None
        try:
            # Found empty constructor: ListLabelledMeasuredArray1D_create_empty
            self.obj = List[LabelledMeasuredArray1D]()
    ...
    def test_fill_value(self):
        if self.obj is None:
            pytest.skip('Skipping test because object could not be instantiated')
        try:
            self.obj.fill_value(1, LabelledMeasuredArray1D.from_farray(FArray[float].from_list([1.0, 2.0, 3.0]), AcquisitionContext.new(Connection.new_barrier('test'), 'test', SymbolUnit.new_meter())))
        except Exception as e:
            print(f'Method call failed: {e}')
    ...
    def test_ctor_new_empty(self):
        try:
            List[LabelledMeasuredArray1D]()
        except Exception as e:
            print(f'Constructor new_empty failed: {e}')

    def test_ctor_new(self):
        try:
            List[LabelledMeasuredArray1D](array.array('L', [0]), 1)
        except Exception as e:
            print(f'Constructor new failed: {e}')

    def test_ctor_from_json(self):
        try:
            List[LabelledMeasuredArray1D]("test_string")
        except Exception as e:
