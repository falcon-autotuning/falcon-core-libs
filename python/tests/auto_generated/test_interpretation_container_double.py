...
class TestInterpretationContainerDouble:
    ...
    def test_insert_or_assign(self):
        if self.obj is None:
            pytest.skip('Skipping test because object could not be instantiated')
        try:
            self.obj.insert_or_assign(InterpretationContext.new(Axes[MeasurementContext](), List[MeasurementContext](), SymbolUnit.new_meter()), 1.0)
        except Exception as e:
            print(f'Method call failed as expected: {e}')
    ...
    def test_insert(self):
        if self.obj is None:
            pytest.skip('Skipping test because object could not be instantiated')
        try:
            self.obj.insert(InterpretationContext.new(Axes[MeasurementContext](), List[MeasurementContext](), SymbolUnit.new_meter()), 1.0)
        except Exception as e:
            print(f'Method call failed as expected: {e}')
