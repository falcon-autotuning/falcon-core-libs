...
class _LabelledArraysFactory:
    """Factory for LabelledArrays[T] instances."""

    def __init__(self, element_type, c_labelled_arrays_class):
        self.element_type = element_type
        self._c_class = c_labelled_arrays_class
    ...
    def __call__(self, *args, **kwargs):
        ...
        if not args and not kwargs:
            ...
            if hasattr(self._c_class, 'new_empty'):
                return LabelledArrays(self._c_class.new_empty(), self.element_type)
            elif hasattr(self._c_class, 'create_empty'):
                return LabelledArrays(self._c_class.create_empty(), self.element_type)
            elif hasattr(self._c_class, 'new'):
                try:
                    return LabelledArrays(self._c_class.new(), self.element_type)
                except TypeError:
                    pass
    ...
    def __getattr__(self, name):
        ...
        if attr is None:
            raise AttributeError(f"LabelledArrays[{self.element_type}] has no attribute {name!r}")
        ...
        if callable(attr):
            ...
            def wrapper(*args, **kwargs):
                ...
                if result is not None and hasattr(result, 'handle'):
                    return LabelledArrays(result, self.element_type)
                return result
            return wrapper
        return attr

class LabelledArrays:
    """Generic LabelledArrays wrapper with full method support."""
    ...
    def __getattr__(self, name):
        ...
        if attr is None:
            raise AttributeError(f"LabelledArrays has no attribute {name!r}")
            ...
            def wrapper(*args, **kwargs):
                ...
                if result is not None and hasattr(result, 'handle'):
                    ...
                    if type(result).__name__ == type(self._c).__name__:
                        return LabelledArrays(result, self._element_type)
                return result
            return wrapper
        return attr
    ...
    def __add__(self, other):
        ...
        if isinstance(other, LabelledArrays) and hasattr(self._c, 'plus_farray'):
    ...
    def __sub__(self, other):
        # Try different method overloads based on argument type
        if isinstance(other, LabelledArrays) and hasattr(self._c, 'minus_farray'):
    ...
    def __mul__(self, other):
        # Try different method overloads based on argument type
        if isinstance(other, LabelledArrays) and hasattr(self._c, 'times_farray'):
    ...
    def __truediv__(self, other):
        # Try different method overloads based on argument type
        if isinstance(other, LabelledArrays) and hasattr(self._c, 'divides_farray'):
    ...
    def __eq__(self, other):
        # Try different method overloads based on argument type
        if isinstance(other, LabelledArrays) and hasattr(self._c, 'equality'):
            return self._c.equality(other._c)
        if isinstance(other, LabelledArrays) and hasattr(self._c, 'equal'):
            return self._c.equal(other._c)
        return NotImplemented

    def __ne__(self, other):
        # Try different method overloads based on argument type
        if isinstance(other, LabelledArrays) and hasattr(self._c, 'notequality'):
            return self._c.notequality(other._c)
        if isinstance(other, LabelledArrays) and hasattr(self._c, 'not_equal'):
    ...
    def __getitem__(self, key):
        ...
        if hasattr(self._c, 'at'):
            ret = self._c.at(key)
            if ret is None:
                exc = KeyError if "LabelledArrays" == "Map" else IndexError
