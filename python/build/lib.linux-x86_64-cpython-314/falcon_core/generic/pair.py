    ...
    def __call__(self, *args, **kwargs):
        ...
        if len(args) == 1 and not kwargs:
            ...
            # Copy constructor or similar - unwrap the argument
                ...
                return Pair(self._c_class.new(unwrap(arg)), self.element_type)
                
        # Pair constructor
        if "Pair" == "Pair" and len(args) == 2:
            if hasattr(self._c_class, 'new'):
                unwrapped_args = [unwrap(a) for a in args]
                return Pair(self._c_class.new(*unwrapped_args), self.element_type)
...
class Pair:
    ...
    def __eq__(self, other):
        # Try different method overloads based on argument type
        if isinstance(other, Pair) and hasattr(self._c, 'equality'):
            return self._c.equality(other._c)
        if isinstance(other, Pair) and hasattr(self._c, 'equal'):
            return self._c.equal(other._c)
        return NotImplemented

    def __ne__(self, other):
        # Try different method overloads based on argument type
        if isinstance(other, Pair) and hasattr(self._c, 'notequality'):
            return self._c.notequality(other._c)
        if isinstance(other, Pair) and hasattr(self._c, 'not_equal'):
    ...
    def __getitem__(self, key):
        ...
        if hasattr(self._c, 'at'):
            ret = self._c.at(key)
            if ret is None:
                exc = KeyError if "Pair" == "Map" else IndexError
