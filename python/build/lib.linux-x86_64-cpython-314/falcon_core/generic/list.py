    ...
    def __call__(self, *args, **kwargs):
        ...
        if not args and not kwargs:
            ...
            if hasattr(self._c_class, 'new_empty'):
                return List(self._c_class.new_empty(), self.element_type)
        ...
        if len(args) == 1 and not kwargs:
            ...
            # Copy constructor or similar - unwrap the argument
                ...
                return List(self._c_class.new(unwrap(arg)), self.element_type)
                
        # Pair constructor
        if "List" == "Pair" and len(args) == 2:
            if hasattr(self._c_class, 'new'):
                unwrapped_args = [unwrap(a) for a in args]
                return List(self._c_class.new(*unwrapped_args), self.element_type)
...
class List:
    ...
    def __eq__(self, other):
        # Try different method overloads based on argument type
        if isinstance(other, List) and hasattr(self._c, 'equality'):
            return self._c.equality(other._c)
        if isinstance(other, List) and hasattr(self._c, 'equal'):
            return self._c.equal(other._c)
        return NotImplemented

    def __ne__(self, other):
        # Try different method overloads based on argument type
        if isinstance(other, List) and hasattr(self._c, 'notequality'):
            return self._c.notequality(other._c)
        if isinstance(other, List) and hasattr(self._c, 'not_equal'):
    ...
    def __getitem__(self, key):
        ...
        if hasattr(self._c, 'at'):
            ret = self._c.at(key)
            if ret is None:
                exc = KeyError if "List" == "Map" else IndexError
