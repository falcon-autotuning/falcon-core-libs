...
def generate_generic_wrapper(base, instances):
    ...
    lines.append(f"        # Helper to unwrap wrapper objects to their underlying C objects")
    lines.append(f"        def unwrap(obj):")
    lines.append(f"            if hasattr(obj, '_c'):")
    lines.append(f"                return obj._c")
    ...
    lines.append(f"            if hasattr(self, 'from_list') and not isinstance(arg, dict) and not hasattr(arg, '_c'):")
    ...
    lines.append(f"            # Copy constructor or similar - unwrap the argument")
    lines.append(f"            elif hasattr(self._c_class, 'new'):")
    lines.append(f"                return {base}(self._c_class.new(unwrap(arg)), self.element_type)")
    lines.append(f"                ")
    lines.append(f"        # Pair constructor")
    lines.append(f'        if "{base}" == "Pair" and len(args) == 2:')
    lines.append(f"            if hasattr(self._c_class, 'new'):")
    lines.append(f"                unwrapped_args = [unwrap(a) for a in args]")
    lines.append(f"                return {base}(self._c_class.new(*unwrapped_args), self.element_type)")
    lines.append("")
    lines.append(f"        # Fallback to raising error if no suitable constructor found")
    lines.append("        raise TypeError(f'No suitable constructor found for " + base + "[{self.element_type}] with args={args}')")
    lines.append("")
    
    # Support from_dict for map-like types
    if base in ["Map", "Pair", "InterpretationContainer"]:
    ...
    lines.append(f"                # Unwrap arguments to their Cython objects")
    lines.append(f"                unwrapped_args = [unwrap(a) for a in args]")
    ...
    lines.append(f"        if hasattr(self._c, '__setitem__'):")
    lines.append(f"            self._c[key] = value")
    lines.append(f"        elif hasattr(self._c, 'insert_or_assign'):")
    lines.append(f"            self._c.insert_or_assign(key, value)")
    lines.append(f"        elif hasattr(self._c, 'insert'):")
    lines.append(f"            self._c.insert(key, value)")
    lines.append(f"        else:")
