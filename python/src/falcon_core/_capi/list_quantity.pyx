cimport _c_api
from cpython.bytes cimport PyBytes_FromStringAndSize
from libc.stddef cimport size_t
from . cimport quantity

cdef class ListQuantity:
    def __cinit__(self):
        self.handle = <_c_api.ListQuantityHandle>0
        self.owned = False

    def __dealloc__(self):
        if self.handle != <_c_api.ListQuantityHandle>0 and self.owned:
            _c_api.ListQuantity_destroy(self.handle)
        self.handle = <_c_api.ListQuantityHandle>0


cdef ListQuantity _list_quantity_from_capi(_c_api.ListQuantityHandle h):
    if h == <_c_api.ListQuantityHandle>0:
        return None
    cdef ListQuantity obj = ListQuantity.__new__(ListQuantity)
    obj.handle = h
    obj.owned = True
    return obj

    @classmethod
    def empty(cls, ):
        cdef _c_api.ListQuantityHandle h
        h = _c_api.ListQuantity_create_empty()
        if h == <_c_api.ListQuantityHandle>0:
            raise MemoryError("Failed to create ListQuantity")
        cdef ListQuantity obj = <ListQuantity>cls.__new__(cls)
        obj.handle = h
        obj.owned = True
        return obj

    @classmethod
    def create(cls, Quantity data, size_t count):
        cdef _c_api.ListQuantityHandle h
        h = _c_api.ListQuantity_create(data.handle, count)
        if h == <_c_api.ListQuantityHandle>0:
            raise MemoryError("Failed to create ListQuantity")
        cdef ListQuantity obj = <ListQuantity>cls.__new__(cls)
        obj.handle = h
        obj.owned = True
        return obj

    @classmethod
    def from_json_string(cls, str json):
        cdef bytes b_json = json.encode("utf-8")
        cdef StringHandle s_json = _c_api.String_create(b_json, len(b_json))
        cdef _c_api.ListQuantityHandle h
        try:
            h = _c_api.ListQuantity_from_json_string(s_json)
        finally:
            _c_api.String_destroy(s_json)
        if h == <_c_api.ListQuantityHandle>0:
            raise MemoryError("Failed to create ListQuantity")
        cdef ListQuantity obj = <ListQuantity>cls.__new__(cls)
        obj.handle = h
        obj.owned = True
        return obj

    @staticmethod
    def fill_value(size_t count, Quantity value):
        cdef _c_api.ListQuantityHandle h_ret = _c_api.ListQuantity_fill_value(count, value.handle)
        if h_ret == <_c_api.ListQuantityHandle>0:
            return None
        return _list_quantity_from_capi(h_ret)

    def push_back(self, Quantity value):
        _c_api.ListQuantity_push_back(self.handle, value.handle)

    def size(self, ):
        return _c_api.ListQuantity_size(self.handle)

    def empty(self, ):
        return _c_api.ListQuantity_empty(self.handle)

    def erase_at(self, size_t idx):
        _c_api.ListQuantity_erase_at(self.handle, idx)

    def clear(self, ):
        _c_api.ListQuantity_clear(self.handle)

    def at(self, size_t idx):
        cdef _c_api.QuantityHandle h_ret = _c_api.ListQuantity_at(self.handle, idx)
        if h_ret == <_c_api.QuantityHandle>0:
            return None
        return quantity._quantity_from_capi(h_ret)

    def items(self, Quantity out_buffer, size_t buffer_size):
        return _c_api.ListQuantity_items(self.handle, out_buffer.handle, buffer_size)

    def contains(self, Quantity value):
        return _c_api.ListQuantity_contains(self.handle, value.handle)

    def index(self, Quantity value):
        return _c_api.ListQuantity_index(self.handle, value.handle)

    def intersection(self, ListQuantity other):
        cdef _c_api.ListQuantityHandle h_ret = _c_api.ListQuantity_intersection(self.handle, other.handle)
        if h_ret == <_c_api.ListQuantityHandle>0:
            return None
        return _list_quantity_from_capi(h_ret)

    def equal(self, ListQuantity b):
        return _c_api.ListQuantity_equal(self.handle, b.handle)

    def __eq__(self, ListQuantity b):
        if not hasattr(b, "handle"):
            return NotImplemented
        return self.equal(b)

    def not_equal(self, ListQuantity b):
        return _c_api.ListQuantity_not_equal(self.handle, b.handle)

    def __ne__(self, ListQuantity b):
        if not hasattr(b, "handle"):
            return NotImplemented
        return self.not_equal(b)
