...
from .list_bool cimport ListBool, _list_bool_from_capi
from .list_pair_string_bool cimport ListPairStringBool, _list_pair_string_bool_from_capi
...
cdef class MapStringBool:
    ...
    def new(cls, size_t[:] data, size_t count):
        cdef _c_api.MapStringBoolHandle h
        h = _c_api.MapStringBool_create(<_c_api.PairStringBoolHandle*>&data[0], count)
        if h == <_c_api.MapStringBoolHandle>0:
            raise MemoryError("Failed to create MapStringBool")
        cdef MapStringBool obj = <MapStringBool>cls.__new__(cls)
        obj.handle = h
    ...
    def insert(self, str key, bint value):
        cdef bytes b_key = key.encode("utf-8")
        cdef _c_api.StringHandle s_key = _c_api.String_create(b_key, len(b_key))
    ...
    def values(self):
        ...
        if h_ret == <_c_api.ListBoolHandle>0: return None
    ...
    def items(self):
        ...
        if h_ret == <_c_api.ListPairStringBoolHandle>0: return None
    ...
    def to_json(self):
        ...
        s_ret = _c_api.MapStringBool_to_json_string(self.handle)
...
cdef MapStringBool _map_string_bool_from_capi(_c_api.MapStringBoolHandle h, bint owned=True):
    if h == <_c_api.MapStringBoolHandle>0:
        return None
