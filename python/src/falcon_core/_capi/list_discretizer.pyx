...
cdef class ListDiscretizer:
    def __cinit__(self):
        self.handle = <_c_api.ListDiscretizerHandle>0
        self.owned = False

    def __dealloc__(self):
        if self.handle != <_c_api.ListDiscretizerHandle>0 and self.owned:
            _c_api.ListDiscretizer_destroy(self.handle)
        self.handle = <_c_api.ListDiscretizerHandle>0
...
cdef ListDiscretizer _list_discretizer_from_capi(_c_api.ListDiscretizerHandle h):
    ...
    @classmethod
    def empty(cls, ):
        cdef _c_api.ListDiscretizerHandle h
        h = _c_api.ListDiscretizer_create_empty()
        if h == <_c_api.ListDiscretizerHandle>0:
    ...
    @classmethod
    def create(cls, Discretizer data, size_t count):
        cdef _c_api.ListDiscretizerHandle h
        h = _c_api.ListDiscretizer_create(data.handle, count)
        if h == <_c_api.ListDiscretizerHandle>0:
            raise MemoryError("Failed to create ListDiscretizer")
        cdef ListDiscretizer obj = <ListDiscretizer>cls.__new__(cls)
        obj.handle = h
    ...
    def from_json_string(cls, str json):
        ...
        cdef _c_api.ListDiscretizerHandle h
        try:
            h = _c_api.ListDiscretizer_from_json_string(s_json)
        finally:
    ...
    def fill_value(size_t count, Discretizer value):
        cdef _c_api.ListDiscretizerHandle h_ret = _c_api.ListDiscretizer_fill_value(count, value.handle)
        if h_ret == <_c_api.ListDiscretizerHandle>0:
            return None
        return _list_discretizer_from_capi(h_ret)

    def push_back(self, Discretizer value):
        _c_api.ListDiscretizer_push_back(self.handle, value.handle)
    ...
    def items(self, Discretizer out_buffer, size_t buffer_size):
        return _c_api.ListDiscretizer_items(self.handle, out_buffer.handle, buffer_size)

    def contains(self, Discretizer value):
        return _c_api.ListDiscretizer_contains(self.handle, value.handle)

    def index(self, Discretizer value):
        return _c_api.ListDiscretizer_index(self.handle, value.handle)

    def intersection(self, ListDiscretizer other):
        cdef _c_api.ListDiscretizerHandle h_ret = _c_api.ListDiscretizer_intersection(self.handle, other.handle)
        if h_ret == <_c_api.ListDiscretizerHandle>0:
            return None
        return _list_discretizer_from_capi(h_ret)

    def equal(self, ListDiscretizer b):
        return _c_api.ListDiscretizer_equal(self.handle, b.handle)
    ...
    def not_equal(self, ListDiscretizer b):
        return _c_api.ListDiscretizer_not_equal(self.handle, b.handle)
