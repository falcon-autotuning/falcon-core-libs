...
cdef Channels _channels_from_capi(_c_api.ChannelsHandle h):
    ...
    def intersection(self, Channels other):
        cdef _c_api.ChannelsHandle h_ret = _c_api.Channels_intersection(self.handle, other.handle)
        if h_ret == <_c_api.ChannelsHandle>0:
            return None
        return _channels_from_capi(h_ret)

    def push_back(self, Channel value):
        _c_api.Channels_push_back(self.handle, value.handle)
    ...
    def contains(self, Channel value):
        return _c_api.Channels_contains(self.handle, value.handle)

    def index(self, Channel value):
        return _c_api.Channels_index(self.handle, value.handle)

    def equal(self, Channels b):
        return _c_api.Channels_equal(self.handle, b.handle)
    ...
    def not_equal(self, Channels b):
        return _c_api.Channels_not_equal(self.handle, b.handle)
