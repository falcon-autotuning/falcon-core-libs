...
cdef LeftReservoirWithImplantedOhmic _left_reservoir_with_implanted_ohmic_from_capi(_c_api.LeftReservoirWithImplantedOhmicHandle h):
    ...
    @classmethod
    def create(cls, str name, Connection right_neighbor, Connection ohmic):
        ...
        try:
            h = _c_api.LeftReservoirWithImplantedOhmic_create(s_name, right_neighbor.handle, ohmic.handle)
        finally:
    ...
    def equal(self, LeftReservoirWithImplantedOhmic b):
        return _c_api.LeftReservoirWithImplantedOhmic_equal(self.handle, b.handle)

    def __eq__(self, LeftReservoirWithImplantedOhmic b):
        if not hasattr(b, "handle"):
            return NotImplemented
        return self.equal(b)

    def not_equal(self, LeftReservoirWithImplantedOhmic b):
        return _c_api.LeftReservoirWithImplantedOhmic_not_equal(self.handle, b.handle)

    def __ne__(self, LeftReservoirWithImplantedOhmic b):
        if not hasattr(b, "handle"):
            return NotImplemented
