...
class _FArrayFactory:
    """Factory for FArray[T] instances."""

    def __init__(self, element_type, c_f_array_class):
        self.element_type = element_type
        self._c_class = c_f_array_class
    ...
    def __call__(self, *args, **kwargs):
        ...
        if len(args) == 1 and not kwargs:
            ...
            # Copy constructor or similar - unwrap the argument
                ...
                return FArray(self._c_class.new(unwrap(arg)), self.element_type)
                
        # Pair constructor
        if "FArray" == "Pair" and len(args) == 2:
            if hasattr(self._c_class, 'new'):
                unwrapped_args = [unwrap(a) for a in args]
                return FArray(self._c_class.new(*unwrapped_args), self.element_type)
    ...
    def __getattr__(self, name):
        ...
        if attr is None:
            raise AttributeError(f"FArray[{self.element_type}] has no attribute {name!r}")
        ...
        if callable(attr):
            ...
            def wrapper(*args, **kwargs):
                ...
                if result is not None and hasattr(result, 'handle'):
                    return FArray(result, self.element_type)
                return result
            return wrapper
        return attr

class FArray:
    """Generic FArray wrapper with full method support."""
    ...
    def __class_getitem__(cls, types):
        ...
        c_class = FARRAY_REGISTRY.get(resolved_types)
        if c_class is None:
            raise TypeError(f"FArray does not support type: {types}")
        return _FArrayFactory(types, c_class)
    ...
    def __getattr__(self, name):
        ...
        if attr is None:
            raise AttributeError(f"FArray has no attribute {name!r}")
        
        # If it's a method, wrap it to handle return values
        if callable(attr):
            def wrapper(*args, **kwargs):
                # Unwrap FArray arguments to their Cython objects
                unwrapped_args = []
                for arg in args:
                    if hasattr(arg, '_c'):
                ...
                if result is not None and hasattr(result, 'handle'):
                    ...
                    if type(result).__name__ == type(self._c).__name__:
                        return FArray(result, self._element_type)
                return result
            return wrapper
        return attr
    ...
    def __add__(self, other):
        ...
        if isinstance(other, FArray) and hasattr(self._c, 'plus_farray'):
    ...
    def __sub__(self, other):
        # Try different method overloads based on argument type
        if isinstance(other, FArray) and hasattr(self._c, 'minus_farray'):
    ...
    def __mul__(self, other):
        # Try different method overloads based on argument type
        if isinstance(other, FArray) and hasattr(self._c, 'times_farray'):
    ...
    def __truediv__(self, other):
        # Try different method overloads based on argument type
        if isinstance(other, FArray) and hasattr(self._c, 'divides_farray'):
    ...
    def __eq__(self, other):
        # Try different method overloads based on argument type
        if isinstance(other, FArray) and hasattr(self._c, 'equality'):
            return self._c.equality(other._c)
        if isinstance(other, FArray) and hasattr(self._c, 'equal'):
            return self._c.equal(other._c)
        return NotImplemented

    def __ne__(self, other):
        # Try different method overloads based on argument type
        if isinstance(other, FArray) and hasattr(self._c, 'notequality'):
            return self._c.notequality(other._c)
        if isinstance(other, FArray) and hasattr(self._c, 'not_equal'):
    ...
    def __getitem__(self, key):
        ...
        if hasattr(self._c, 'at'):
            ret = self._c.at(key)
            if ret is None:
                exc = KeyError if "FArray" == "Map" else IndexError
