...
// --- Helper functions for minimal valid objects ---
...
func TestMeasurementRequest_Accessors(t *testing.T) {
	...
	if err != nil {
		t.Errorf("Waveforms error: %v", err)
	}
	waves.Close()
	gets, err := req.Getters()
	if err != nil {
		t.Errorf("Getters error: %v", err)
	}
	gets.Close()
	mt, err := req.MeterTransforms()
	if err != nil {
		t.Errorf("MeterTransforms error: %v", err)
	}
	mt.Close()
	td, err := req.TimeDomain()
	if err != nil {
		t.Errorf("TimeDomain error: %v", err)
	}
	td.Close()

	// Nil/closed error branches
	req.Close()
	...
	if _, err := req.MeterTransforms(); err == nil {
		t.Errorf("MeterTransforms on closed should error")
	}
	if _, err := req.TimeDomain(); err == nil {
		t.Errorf("TimeDomain on closed should error")
	}
}
