...
func mustPairInterpretationContextQuantity(ctx *interpretationcontext.Handle, val *quantity.Handle) *pairinterpretationcontextquantity.Handle {
	h, err := pairinterpretationcontextquantity.New(ctx, val)
	if err != nil {
		panic(fmt.Errorf("failed to create pairinterpretationcontextquantity: %v", err))
	}
	return h
}

func mustMapInterpretationContextQuantity(ctx *interpretationcontext.Handle, val *quantity.Handle) *mapinterpretationcontextquantity.Handle {
	h, err := mapinterpretationcontextquantity.New([]*pairinterpretationcontextquantity.Handle{
		mustPairInterpretationContextQuantity(ctx, val),
	})
	if err != nil {
		panic(err)
	}
	if h == nil {
		panic(fmt.Errorf("failed to create mapinterpretationcontextquantity"))
	}
	return h
}

var (
	testValue               = mustQuantity(0.1)
	...
	testMapHandle           = mustMapInterpretationContextQuantity(testInterpretationCtx, testValue)
)
