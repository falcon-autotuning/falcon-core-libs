...
func Copy(handle *Handle) (*Handle, error) {
	return cmemoryallocation.Read(handle, func() (*Handle, error) {
		...
		return cmemoryallocation.NewAllocation(
			func() (unsafe.Pointer, error) {
				return unsafe.Pointer(C.InterpretationContainerString_copy(C.InterpretationContainerStringHandle(handle.CAPIHandle()))), nil
			},
			construct,
			destroy,
		)
...
func (h *Handle) Size() (uint64, error) {
	return cmemoryallocation.Read(h, func() (uint64, error) {
		return uint64(C.InterpretationContainerString_size(C.InterpretationContainerStringHandle(h.CAPIHandle()))), nil
	})
}
