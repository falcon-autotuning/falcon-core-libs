...
func mustPairInterpretationContextString(ctx *interpretationcontext.Handle, val string) *pairinterpretationcontextstring.Handle {
	h, err := pairinterpretationcontextstring.New(ctx, val)
	if err != nil {
		panic(fmt.Errorf("failed to create pairinterpretationcontextstring: %v", err))
	}
	return h
}

func mustMapInterpretationContextString(ctx *interpretationcontext.Handle, val string) *mapinterpretationcontextstring.Handle {
	h, err := mapinterpretationcontextstring.New([]*pairinterpretationcontextstring.Handle{
		mustPairInterpretationContextString(ctx, val),
	})
	if err != nil {
		panic(err)
	}
	if h == nil {
		panic(fmt.Errorf("failed to create mapinterpretationcontextstring"))
	}
	return h
}
...
var (
	...
	testMapHandle           = mustMapInterpretationContextString(testInterpretationCtx, testValue)
)
