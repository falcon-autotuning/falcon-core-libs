...
func New(data []*pairintfloat.Handle) (*Handle, error) {
	...
	cData := C.malloc(C.size_t(nData) * C.size_t(unsafe.Sizeof(C.PairIntFloatHandle(nil))))
	if cData == nil {
		return nil, errors.New("C.malloc failed")
	}
	slicecData := (*[1 << 30]C.PairIntFloatHandle)(cData)[:nData:nData]
	for i, v := range data {
		slicecData[i] = C.PairIntFloatHandle(v.CAPIHandle())
	}
	return cmemoryallocation.NewAllocation(
		func() (unsafe.Pointer, error) {
			res := unsafe.Pointer(C.ListPairIntFloat_create((*C.PairIntFloatHandle)(cData), C.size_t(nData)))
			C.free(cData)
			return res, nil
		},
		construct,
