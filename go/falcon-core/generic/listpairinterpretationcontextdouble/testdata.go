...
func mustPairInterpretationContextDouble(int *interpretationcontext.Handle, val float64) *pairinterpretationcontextdouble.Handle {
	h, err := pairinterpretationcontextdouble.New(int, val)
	if err != nil {
		panic(err)
	}
	return h
...
var (
	defaultListData = []*pairinterpretationcontextdouble.Handle{
		mustPairInterpretationContextDouble(mustInterpretationContext([]string{"a", "b"}, []string{"c", "d"}),
			1.0),
		mustPairInterpretationContextDouble(mustInterpretationContext([]string{"d"}, []string{"c", "e"}),
			1.4),
	}
	otherListData = []*pairinterpretationcontextdouble.Handle{
		mustPairInterpretationContextDouble(mustInterpretationContext([]string{"e"}, []string{"c", "d"}),
			1.4),
	}
)
