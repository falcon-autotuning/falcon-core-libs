...
func New(data []*labelledmeasuredarray.Handle) (*Handle, error) {
	...
	cData := C.malloc(C.size_t(nData) * C.size_t(unsafe.Sizeof(C.LabelledMeasuredArrayHandle(nil))))
	if cData == nil {
		return nil, errors.New("C.malloc failed")
	}
	slicecData := (*[1 << 30]C.LabelledMeasuredArrayHandle)(cData)[:nData:nData]
	for i, v := range data {
		slicecData[i] = C.LabelledMeasuredArrayHandle(v.CAPIHandle())
	}
	return cmemoryallocation.NewAllocation(
		func() (unsafe.Pointer, error) {
			res := unsafe.Pointer(C.ListLabelledMeasuredArray_create((*C.LabelledMeasuredArrayHandle)(cData), C.size_t(nData)))
			C.free(cData)
			return res, nil
		},
		construct,
