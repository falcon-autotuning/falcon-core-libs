...
func New(data []*pairstringstring.Handle) (*Handle, error) {
	...
	cData := C.malloc(C.size_t(nData) * C.size_t(unsafe.Sizeof(C.PairStringStringHandle(nil))))
	if cData == nil {
		return nil, errors.New("C.malloc failed")
	}
	slicecData := (*[1 << 30]C.PairStringStringHandle)(cData)[:nData:nData]
	for i, v := range data {
		slicecData[i] = C.PairStringStringHandle(v.CAPIHandle())
	}
	return cmemoryallocation.NewAllocation(
		func() (unsafe.Pointer, error) {
			res := unsafe.Pointer(C.ListPairStringString_create((*C.PairStringStringHandle)(cData), C.size_t(nData)))
			C.free(cData)
			return res, nil
		},
		construct,
