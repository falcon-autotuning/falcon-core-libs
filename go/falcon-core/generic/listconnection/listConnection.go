...
func New(data []*connection.Handle) (*Handle, error) {
	...
	size := C.size_t(n) * C.size_t(unsafe.Sizeof(C.ConnectionHandle(nil)))
	cList := C.malloc(size)
	if cList == nil {
		return nil, errors.New("C.malloc failed")
	}
	// Copy Go data to C memory
	slice := (*[1 << 30]C.ConnectionHandle)(cList)[:n:n]
	for i, v := range data {
		slice[i] = C.ConnectionHandle(v.CAPIHandle())
	}
	return cmemoryallocation.NewAllocation(
		func() (unsafe.Pointer, error) {
			res := unsafe.Pointer(C.ListConnection_create((*C.ConnectionHandle)(cList), C.size_t(n)))
			C.free(cList)
			return res, nil
		},
		construct,
