...
func New(data []*pairinstrumentportporttransform.Handle) (*Handle, error) {
	...
	cData := C.malloc(C.size_t(nData) * C.size_t(unsafe.Sizeof(C.PairInstrumentPortPortTransformHandle(nil))))
	if cData == nil {
		return nil, errors.New("C.malloc failed")
	}
	slicecData := (*[1 << 30]C.PairInstrumentPortPortTransformHandle)(cData)[:nData:nData]
	for i, v := range data {
		slicecData[i] = C.PairInstrumentPortPortTransformHandle(v.CAPIHandle())
	}
	return cmemoryallocation.NewAllocation(
		func() (unsafe.Pointer, error) {
			res := unsafe.Pointer(C.ListPairInstrumentPortPortTransform_create((*C.PairInstrumentPortPortTransformHandle)(cData), C.size_t(nData)))
			C.free(cData)
			return res, nil
		},
		construct,
