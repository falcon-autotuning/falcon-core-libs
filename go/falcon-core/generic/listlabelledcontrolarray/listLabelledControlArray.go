...
func New(data []*labelledcontrolarray.Handle) (*Handle, error) {
	...
	size := C.size_t(n) * C.size_t(unsafe.Sizeof(C.LabelledControlArrayHandle(nil)))
	cList := C.malloc(size)
	if cList == nil {
		return nil, errors.New("C.malloc failed")
	}
	// Copy Go data to C memory
	slice := (*[1 << 30]C.LabelledControlArrayHandle)(cList)[:n:n]
	for i, v := range data {
		slice[i] = C.LabelledControlArrayHandle(v.CAPIHandle())
	}
	return cmemoryallocation.NewAllocation(
		func() (unsafe.Pointer, error) {
			res := unsafe.Pointer(C.ListLabelledControlArray_create((*C.LabelledControlArrayHandle)(cList), C.size_t(n)))
			C.free(cList)
			return res, nil
		},
		construct,
