...
func New(data []*pairconnectiondouble.Handle) (*Handle, error) {
	...
	cData := C.malloc(C.size_t(nData) * C.size_t(unsafe.Sizeof(C.PairConnectionDoubleHandle(nil))))
	if cData == nil {
		return nil, errors.New("C.malloc failed")
	}
	slicecData := (*[1 << 30]C.PairConnectionDoubleHandle)(cData)[:nData:nData]
	for i, v := range data {
		slicecData[i] = C.PairConnectionDoubleHandle(v.CAPIHandle())
	}
	return cmemoryallocation.NewAllocation(
		func() (unsafe.Pointer, error) {
			res := unsafe.Pointer(C.ListPairConnectionDouble_create((*C.PairConnectionDoubleHandle)(cData), C.size_t(nData)))
			C.free(cData)
			return res, nil
		},
		construct,
