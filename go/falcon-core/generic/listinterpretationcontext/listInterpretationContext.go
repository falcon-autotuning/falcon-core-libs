...
func New(data []*interpretationcontext.Handle) (*Handle, error) {
	...
	cData := C.malloc(C.size_t(nData) * C.size_t(unsafe.Sizeof(C.InterpretationContextHandle(nil))))
	if cData == nil {
		return nil, errors.New("C.malloc failed")
	}
	slicecData := (*[1 << 30]C.InterpretationContextHandle)(cData)[:nData:nData]
	for i, v := range data {
		slicecData[i] = C.InterpretationContextHandle(v.CAPIHandle())
	}
	return cmemoryallocation.NewAllocation(
		func() (unsafe.Pointer, error) {
			res := unsafe.Pointer(C.ListInterpretationContext_create((*C.InterpretationContextHandle)(cData), C.size_t(nData)))
			C.free(cData)
			return res, nil
		},
		construct,
