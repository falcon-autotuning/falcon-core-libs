...
func New(data []*acquisitioncontext.Handle) (*Handle, error) {
	...
	cData := C.malloc(C.size_t(nData) * C.size_t(unsafe.Sizeof(C.AcquisitionContextHandle(nil))))
	if cData == nil {
		return nil, errors.New("C.malloc failed")
	}
	slicecData := (*[1 << 30]C.AcquisitionContextHandle)(cData)[:nData:nData]
	for i, v := range data {
		slicecData[i] = C.AcquisitionContextHandle(v.CAPIHandle())
	}
	return cmemoryallocation.NewAllocation(
		func() (unsafe.Pointer, error) {
			res := unsafe.Pointer(C.ListAcquisitionContext_create((*C.AcquisitionContextHandle)(cData), C.size_t(nData)))
			C.free(cData)
			return res, nil
		},
		construct,
