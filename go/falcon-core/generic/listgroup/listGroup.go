...
func New(data []*group.Handle) (*Handle, error) {
	...
	cData := C.malloc(C.size_t(nData) * C.size_t(unsafe.Sizeof(C.GroupHandle(nil))))
	if cData == nil {
		return nil, errors.New("C.malloc failed")
	}
	slicecData := (*[1 << 30]C.GroupHandle)(cData)[:nData:nData]
	for i, v := range data {
		slicecData[i] = C.GroupHandle(v.CAPIHandle())
	}
	return cmemoryallocation.NewAllocation(
		func() (unsafe.Pointer, error) {
			res := unsafe.Pointer(C.ListGroup_create((*C.GroupHandle)(cData), C.size_t(nData)))
			C.free(cData)
			return res, nil
		},
		construct,
