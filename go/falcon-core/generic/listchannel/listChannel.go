...
func New(data []*channel.Handle) (*Handle, error) {
	...
	cData := C.malloc(C.size_t(nData) * C.size_t(unsafe.Sizeof(C.ChannelHandle(nil))))
	if cData == nil {
		return nil, errors.New("C.malloc failed")
	}
	slicecData := (*[1 << 30]C.ChannelHandle)(cData)[:nData:nData]
	for i, v := range data {
		slicecData[i] = C.ChannelHandle(v.CAPIHandle())
	}
	return cmemoryallocation.NewAllocation(
		func() (unsafe.Pointer, error) {
			res := unsafe.Pointer(C.ListChannel_create((*C.ChannelHandle)(cData), C.size_t(nData)))
			C.free(cData)
			return res, nil
		},
		construct,
