...
func mustPairInterpretationContextQuantity(int *interpretationcontext.Handle, val float64) *pairinterpretationcontextquantity.Handle {
	h, err := pairinterpretationcontextquantity.New(int, mustQuantity(val))
	if err != nil {
		panic(err)
	}
	return h
...
var (
	defaultListData = []*pairinterpretationcontextquantity.Handle{
		mustPairInterpretationContextQuantity(mustInterpretationContext([]string{"a", "b"}, []string{"c", "d"}),
			1.0),
		mustPairInterpretationContextQuantity(mustInterpretationContext([]string{"d"}, []string{"c", "e"}),
			1.4),
	}
	otherListData = []*pairinterpretationcontextquantity.Handle{
		mustPairInterpretationContextQuantity(mustInterpretationContext([]string{"e"}, []string{"c", "d"}),
			1.4),
	}
)
