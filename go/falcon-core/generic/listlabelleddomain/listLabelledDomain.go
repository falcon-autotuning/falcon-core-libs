...
func New(data []*labelleddomain.Handle) (*Handle, error) {
	...
	cData := C.malloc(C.size_t(nData) * C.size_t(unsafe.Sizeof(C.LabelledDomainHandle(nil))))
	if cData == nil {
		return nil, errors.New("C.malloc failed")
	}
	slicecData := (*[1 << 30]C.LabelledDomainHandle)(cData)[:nData:nData]
	for i, v := range data {
		slicecData[i] = C.LabelledDomainHandle(v.CAPIHandle())
	}
	return cmemoryallocation.NewAllocation(
		func() (unsafe.Pointer, error) {
			res := unsafe.Pointer(C.ListLabelledDomain_create((*C.LabelledDomainHandle)(cData), C.size_t(nData)))
			C.free(cData)
			return res, nil
		},
		construct,
