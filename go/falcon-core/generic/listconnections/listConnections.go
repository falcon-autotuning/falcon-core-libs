...
func New(data []*connections.Handle) (*Handle, error) {
	...
	size := C.size_t(n) * C.size_t(unsafe.Sizeof(C.ConnectionsHandle(nil)))
	cList := C.malloc(size)
	if cList == nil {
		return nil, errors.New("C.malloc failed")
	}
	// Copy Go data to C memory
	slice := (*[1 << 30]C.ConnectionsHandle)(cList)[:n:n]
	for i, v := range data {
		slice[i] = C.ConnectionsHandle(v.CAPIHandle())
	}
	return cmemoryallocation.NewAllocation(
		func() (unsafe.Pointer, error) {
			res := unsafe.Pointer(C.ListConnections_create((*C.ConnectionsHandle)(cList), C.size_t(n)))
			C.free(cList)
			return res, nil
		},
		construct,
