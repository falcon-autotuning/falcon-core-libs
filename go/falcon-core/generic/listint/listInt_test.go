...
func TestListInt_NewEmptyAndPushBack(t *testing.T) {
	l, err := NewEmpty()
	if err != nil {
...
func TestListInt_AllocateAndFillValue(t *testing.T) {
	l2, err := Allocate(3)
	if err != nil {
...
func TestListInt_ToJSONAndFromJSON(t *testing.T) {
...
func TestListInt_FromCAPI(t *testing.T) {
	h, err := FromCAPI(nil)
	if err == nil || h != nil {
		t.Errorf("FromCAPI(nil) should error and return nil")
	}
}
func TestListInt_CloseTwice(t *testing.T) {
	l, err := New(defaultListData)
	if err != nil {
		t.Fatalf("New failed: %v", err)
	}
