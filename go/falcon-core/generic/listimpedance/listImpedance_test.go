...
func fixtureListData() []*impedance.Handle {
	return defaultListData
}
func TestListImpedance_NewEmptyAndPushBack(t *testing.T) {
	l, err := NewEmpty()
	if err != nil {
...
func TestListImpedance_NewFromSliceAndItems(t *testing.T) {
...
func TestListImpedance_ToJSONAndFromJSON(t *testing.T) {
...
func TestListImpedance_FromCAPI(t *testing.T) {
	h, err := FromCAPI(nil)
	if err == nil || h != nil {
		t.Errorf("FromCAPI(nil) should error and return nil")
	}
}
func TestListImpedance_CloseTwice(t *testing.T) {
	l, err := New(defaultListData)
	if err != nil {
		t.Fatalf("New failed: %v", err)
	}
