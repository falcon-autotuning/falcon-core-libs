...
func New(data []*controlarray1d.Handle) (*Handle, error) {
	...
	cData := C.malloc(C.size_t(nData) * C.size_t(unsafe.Sizeof(C.ControlArray1DHandle(nil))))
	if cData == nil {
		return nil, errors.New("C.malloc failed")
	}
	slicecData := (*[1 << 30]C.ControlArray1DHandle)(cData)[:nData:nData]
	for i, v := range data {
		slicecData[i] = C.ControlArray1DHandle(v.CAPIHandle())
	}
	return cmemoryallocation.NewAllocation(
		func() (unsafe.Pointer, error) {
			res := unsafe.Pointer(C.ListControlArray1D_create((*C.ControlArray1DHandle)(cData), C.size_t(nData)))
			C.free(cData)
			return res, nil
		},
		construct,
