...
func New(data []*pairstringdouble.Handle) (*Handle, error) {
	...
	cData := C.malloc(C.size_t(nData) * C.size_t(unsafe.Sizeof(C.PairStringDoubleHandle(nil))))
	if cData == nil {
		return nil, errors.New("C.malloc failed")
	}
	slicecData := (*[1 << 30]C.PairStringDoubleHandle)(cData)[:nData:nData]
	for i, v := range data {
		slicecData[i] = C.PairStringDoubleHandle(v.CAPIHandle())
	}
	return cmemoryallocation.NewAllocation(
		func() (unsafe.Pointer, error) {
			res := unsafe.Pointer(C.ListPairStringDouble_create((*C.PairStringDoubleHandle)(cData), C.size_t(nData)))
			C.free(cData)
			return res, nil
		},
		construct,
