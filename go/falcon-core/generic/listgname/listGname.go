...
func New(data []*gname.Handle) (*Handle, error) {
	...
	cData := C.malloc(C.size_t(nData) * C.size_t(unsafe.Sizeof(C.GnameHandle(nil))))
	if cData == nil {
		return nil, errors.New("C.malloc failed")
	}
	slicecData := (*[1 << 30]C.GnameHandle)(cData)[:nData:nData]
	for i, v := range data {
		slicecData[i] = C.GnameHandle(v.CAPIHandle())
	}
	return cmemoryallocation.NewAllocation(
		func() (unsafe.Pointer, error) {
			res := unsafe.Pointer(C.ListGname_create((*C.GnameHandle)(cData), C.size_t(nData)))
			C.free(cData)
			return res, nil
		},
		construct,
