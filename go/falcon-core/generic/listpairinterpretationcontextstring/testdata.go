...
func mustPairInterpretationContextString(int *interpretationcontext.Handle, str string) *pairinterpretationcontextstring.Handle {
	h, err := pairinterpretationcontextstring.New(int, str)
	if err != nil {
		panic(err)
	}
	return h
...
var (
	defaultListData = []*pairinterpretationcontextstring.Handle{
		mustPairInterpretationContextString(mustInterpretationContext([]string{"a", "b"}, []string{"c", "d"}),
			"hello"),
		mustPairInterpretationContextString(mustInterpretationContext([]string{"d"}, []string{"c", "e"}),
			"world"),
	}
	otherListData = []*pairinterpretationcontextstring.Handle{
		mustPairInterpretationContextString(mustInterpretationContext([]string{"e"}, []string{"c", "d"}),
			"wow"),
	}
)
