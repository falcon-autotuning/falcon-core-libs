...
func New(data []*dotgatewithneighbors.Handle) (*Handle, error) {
	...
	cData := C.malloc(C.size_t(nData) * C.size_t(unsafe.Sizeof(C.DotGateWithNeighborsHandle(nil))))
	if cData == nil {
		return nil, errors.New("C.malloc failed")
	}
	slicecData := (*[1 << 30]C.DotGateWithNeighborsHandle)(cData)[:nData:nData]
	for i, v := range data {
		slicecData[i] = C.DotGateWithNeighborsHandle(v.CAPIHandle())
	}
	return cmemoryallocation.NewAllocation(
		func() (unsafe.Pointer, error) {
			res := unsafe.Pointer(C.ListDotGateWithNeighbors_create((*C.DotGateWithNeighborsHandle)(cData), C.size_t(nData)))
			C.free(cData)
			return res, nil
		},
		construct,
