...
func New(data []*pairconnectionconnections.Handle) (*Handle, error) {
	...
	cData := C.malloc(C.size_t(nData) * C.size_t(unsafe.Sizeof(C.PairConnectionConnectionsHandle(nil))))
	if cData == nil {
		return nil, errors.New("C.malloc failed")
	}
	slicecData := (*[1 << 30]C.PairConnectionConnectionsHandle)(cData)[:nData:nData]
	for i, v := range data {
		slicecData[i] = C.PairConnectionConnectionsHandle(v.CAPIHandle())
	}
	return cmemoryallocation.NewAllocation(
		func() (unsafe.Pointer, error) {
			res := unsafe.Pointer(C.ListPairConnectionConnections_create((*C.PairConnectionConnectionsHandle)(cData), C.size_t(nData)))
			C.free(cData)
			return res, nil
		},
		construct,
