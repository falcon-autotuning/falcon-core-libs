...
func New(data []*pairconnectionpairquantityquantity.Handle) (*Handle, error) {
	...
	cData := C.malloc(C.size_t(nData) * C.size_t(unsafe.Sizeof(C.PairConnectionPairQuantityQuantityHandle(nil))))
	if cData == nil {
		return nil, errors.New("C.malloc failed")
	}
	slicecData := (*[1 << 30]C.PairConnectionPairQuantityQuantityHandle)(cData)[:nData:nData]
	for i, v := range data {
		slicecData[i] = C.PairConnectionPairQuantityQuantityHandle(v.CAPIHandle())
	}
	return cmemoryallocation.NewAllocation(
		func() (unsafe.Pointer, error) {
			res := unsafe.Pointer(C.ListPairConnectionPairQuantityQuantity_create((*C.PairConnectionPairQuantityQuantityHandle)(cData), C.size_t(nData)))
			C.free(cData)
			return res, nil
		},
		construct,
