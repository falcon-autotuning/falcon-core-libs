...
func New(data []*instrumentport.Handle) (*Handle, error) {
	...
	cData := C.malloc(C.size_t(nData) * C.size_t(unsafe.Sizeof(C.InstrumentPortHandle(nil))))
	if cData == nil {
		return nil, errors.New("C.malloc failed")
	}
	slicecData := (*[1 << 30]C.InstrumentPortHandle)(cData)[:nData:nData]
	for i, v := range data {
		slicecData[i] = C.InstrumentPortHandle(v.CAPIHandle())
	}
	return cmemoryallocation.NewAllocation(
		func() (unsafe.Pointer, error) {
			res := unsafe.Pointer(C.ListInstrumentPort_create((*C.InstrumentPortHandle)(cData), C.size_t(nData)))
			C.free(cData)
			return res, nil
		},
		construct,
