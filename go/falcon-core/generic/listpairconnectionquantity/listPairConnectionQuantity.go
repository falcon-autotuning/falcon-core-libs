...
func New(data []*pairconnectionquantity.Handle) (*Handle, error) {
	...
	cData := C.malloc(C.size_t(nData) * C.size_t(unsafe.Sizeof(C.PairConnectionQuantityHandle(nil))))
	if cData == nil {
		return nil, errors.New("C.malloc failed")
	}
	slicecData := (*[1 << 30]C.PairConnectionQuantityHandle)(cData)[:nData:nData]
	for i, v := range data {
		slicecData[i] = C.PairConnectionQuantityHandle(v.CAPIHandle())
	}
	return cmemoryallocation.NewAllocation(
		func() (unsafe.Pointer, error) {
			res := unsafe.Pointer(C.ListPairConnectionQuantity_create((*C.PairConnectionQuantityHandle)(cData), C.size_t(nData)))
			C.free(cData)
			return res, nil
		},
		construct,
