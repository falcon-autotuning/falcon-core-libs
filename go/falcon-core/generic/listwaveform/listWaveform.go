...
func New(data []*waveform.Handle) (*Handle, error) {
	...
	cData := C.malloc(C.size_t(nData) * C.size_t(unsafe.Sizeof(C.WaveformHandle(nil))))
	if cData == nil {
		return nil, errors.New("C.malloc failed")
	}
	slicecData := (*[1 << 30]C.WaveformHandle)(cData)[:nData:nData]
	for i, v := range data {
		slicecData[i] = C.WaveformHandle(v.CAPIHandle())
	}
	return cmemoryallocation.NewAllocation(
		func() (unsafe.Pointer, error) {
			res := unsafe.Pointer(C.ListWaveform_create((*C.WaveformHandle)(cData), C.size_t(nData)))
			C.free(cData)
			return res, nil
		},
		construct,
