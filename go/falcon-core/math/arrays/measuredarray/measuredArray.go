...
func (h *Handle) Size() (uint32, error) {
	return cmemoryallocation.Read(h, func() (uint32, error) {
		return uint32(C.MeasuredArray_size(C.MeasuredArrayHandle(h.CAPIHandle()))), nil
	})
}
func (h *Handle) Dimension() (uint32, error) {
	return cmemoryallocation.Read(h, func() (uint32, error) {
		return uint32(C.MeasuredArray_dimension(C.MeasuredArrayHandle(h.CAPIHandle()))), nil
	})
}
