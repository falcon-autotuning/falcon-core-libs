...
class c_axesdiscretizer (h : unit ptr) : c_axesdiscretizer_t = object(self)
  val raw_val = h
  method raw = raw_val
...
module AxesDiscretizer = struct
  ...
  let make (data : Listdiscretizer.ListDiscretizer.t) : t =
  ...
  let fromjson (json : string) : t =
    let ptr = Capi_bindings.axesdiscretizer_from_json_string (Falcon_string.of_string json) in
    Error_handling.raise_if_error ();
    new c_axesdiscretizer ptr
