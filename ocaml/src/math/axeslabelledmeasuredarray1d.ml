...
class c_axeslabelledmeasuredarray1d (h : unit ptr) : c_axeslabelledmeasuredarray1d_t = object(self)
  val raw_val = h
  method raw = raw_val
...
module AxesLabelledMeasuredArray1D = struct
  ...
  let fromjson (json : string) : t =
    let ptr = Capi_bindings.axeslabelledmeasuredarray1d_from_json_string (Falcon_string.of_string json) in
    Error_handling.raise_if_error ();
    new c_axeslabelledmeasuredarray1d ptr
