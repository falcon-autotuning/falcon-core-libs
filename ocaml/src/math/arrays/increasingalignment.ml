...
class c_increasingalignment (h : unit ptr) : c_increasingalignment_t = object(self)
  val raw_val = h
  method raw = raw_val
...
module IncreasingAlignment = struct
  ...
  let fromjson (json : string) : t =
    let ptr = Capi_bindings.increasingalignment_from_json_string (Falcon_string.of_string json) in
    Error_handling.raise_if_error ();
    new c_increasingalignment ptr
