...
class c_labelledarrayslabelledmeasuredarray (h : unit ptr) : c_labelledarrayslabelledmeasuredarray_t = object(self)
  val raw_val = h
  method raw = raw_val
...
module LabelledArraysLabelledMeasuredArray = struct
  ...
  let make (arrays : Listlabelledmeasuredarray.ListLabelledMeasuredArray.t) : t =
  ...
  let fromjson (json : string) : t =
    let ptr = Capi_bindings.labelledarrayslabelledmeasuredarray_from_json_string (Falcon_string.of_string json) in
    Error_handling.raise_if_error ();
    new c_labelledarrayslabelledmeasuredarray ptr

  let arrays (handle : t) : Listlabelledmeasuredarray.ListLabelledMeasuredArray.t =
    Error_handling.read handle (fun () ->
      let result = Capi_bindings.labelledarrayslabelledmeasuredarray_arrays handle#raw in
      Error_handling.raise_if_error ();
      new Listlabelledmeasuredarray.c_listlabelledmeasuredarray result
    )
