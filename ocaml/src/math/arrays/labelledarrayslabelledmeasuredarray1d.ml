...
class c_labelledarrayslabelledmeasuredarray1d (h : unit ptr) : c_labelledarrayslabelledmeasuredarray1d_t = object(self)
  val raw_val = h
  method raw = raw_val
...
module LabelledArraysLabelledMeasuredArray1D = struct
  ...
  let make (arrays : Listlabelledmeasuredarray1d.ListLabelledMeasuredArray1D.t) : t =
  ...
  let fromjson (json : string) : t =
    let ptr = Capi_bindings.labelledarrayslabelledmeasuredarray1d_from_json_string (Falcon_string.of_string json) in
    Error_handling.raise_if_error ();
    new c_labelledarrayslabelledmeasuredarray1d ptr

  let arrays (handle : t) : Listlabelledmeasuredarray1d.ListLabelledMeasuredArray1D.t =
    Error_handling.read handle (fun () ->
      let result = Capi_bindings.labelledarrayslabelledmeasuredarray1d_arrays handle#raw in
      Error_handling.raise_if_error ();
      new Listlabelledmeasuredarray1d.c_listlabelledmeasuredarray1d result
    )
