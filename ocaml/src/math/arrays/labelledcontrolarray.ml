...
class c_labelledcontrolarray (h : unit ptr) : c_labelledcontrolarray_t = object(self)
  val raw_val = h
  method raw = raw_val
...
module LabelledControlArray = struct
  ...
  let fromjson (json : string) : t =
    let ptr = Capi_bindings.labelledcontrolarray_from_json_string (Falcon_string.of_string json) in
    Error_handling.raise_if_error ();
    new c_labelledcontrolarray ptr
