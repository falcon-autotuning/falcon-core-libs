...
class c_dotgateswithneighbors (h : unit ptr) : c_dotgateswithneighbors_t = object(self)
  val raw_val = h
  method raw = raw_val
...
module DotGatesWithNeighbors = struct
  ...
  let fromjson (json : string) : t =
    let ptr = Capi_bindings.dotgateswithneighbors_from_json_string (Falcon_string.of_string json) in
    Error_handling.raise_if_error ();
    new c_dotgateswithneighbors ptr
  ...
  let make (items : Listdotgatewithneighbors.ListDotGateWithNeighbors.t) : t =
  ...
  let items (handle : t) : Listdotgatewithneighbors.ListDotGateWithNeighbors.t =
    Error_handling.read handle (fun () ->
      let result = Capi_bindings.dotgateswithneighbors_items handle#raw in
      Error_handling.raise_if_error ();
      new Listdotgatewithneighbors.c_listdotgatewithneighbors result
    )
