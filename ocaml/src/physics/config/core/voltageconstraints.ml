...
class c_voltageconstraints (h : unit ptr) : c_voltageconstraints_t = object(self)
  val raw_val = h
  method raw = raw_val
...
module VoltageConstraints = struct
  ...
  let fromjson (json : string) : t =
    let ptr = Capi_bindings.voltageconstraints_from_json_string (Falcon_string.of_string json) in
    Error_handling.raise_if_error ();
    new c_voltageconstraints ptr
