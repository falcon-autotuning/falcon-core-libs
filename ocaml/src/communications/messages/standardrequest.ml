...
class c_standardrequest (h : unit ptr) : c_standardrequest_t = object(self)
  val raw_val = h
  method raw = raw_val
...
module StandardRequest = struct
  ...
  let fromjson (json : string) : t =
    let ptr = Capi_bindings.standardrequest_from_json_string (Falcon_string.of_string json) in
    Error_handling.raise_if_error ();
    new c_standardrequest ptr

  let make (message : string) : t =
    let ptr = Capi_bindings.standardrequest_create (Falcon_string.of_string message) in
    Error_handling.raise_if_error ();
    new c_standardrequest ptr
