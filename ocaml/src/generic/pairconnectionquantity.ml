...
class c_pairconnectionquantity (h : unit ptr) : c_pairconnectionquantity_t = object(self)
  val raw_val = h
  method raw = raw_val
...
module PairConnectionQuantity = struct
  ...
  let make (first : Connection.Connection.t) (second : Quantity.Quantity.t) : t =
  ...
  let fromjson (json : string) : t =
    let ptr = Capi_bindings.pairconnectionquantity_from_json_string (Falcon_string.of_string json) in
    Error_handling.raise_if_error ();
    new c_pairconnectionquantity ptr
