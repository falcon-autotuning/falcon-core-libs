...
class c_mapinterpretationcontextquantity (h : unit ptr) : c_mapinterpretationcontextquantity_t = object(self)
  val raw_val = h
  method raw = raw_val
...
module MapInterpretationContextQuantity = struct
  ...
  let fromjson (json : string) : t =
    let ptr = Capi_bindings.mapinterpretationcontextquantity_from_json_string (Falcon_string.of_string json) in
    Error_handling.raise_if_error ();
    new c_mapinterpretationcontextquantity ptr
