...
class c_listinstrumentport (h : unit ptr) : c_listinstrumentport_t = object(self)
  val raw_val = h
  method raw = raw_val
...
module ListInstrumentPort = struct
  ...
  let fromjson (json : string) : t =
    let ptr = Capi_bindings.listinstrumentport_from_json_string (Falcon_string.of_string json) in
    Error_handling.raise_if_error ();
    new c_listinstrumentport ptr
