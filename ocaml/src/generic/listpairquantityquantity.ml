...
class c_listpairquantityquantity (h : unit ptr) : c_listpairquantityquantity_t = object(self)
  val raw_val = h
  method raw = raw_val
...
module ListPairQuantityQuantity = struct
  ...
  let fromjson (json : string) : t =
    let ptr = Capi_bindings.listpairquantityquantity_from_json_string (Falcon_string.of_string json) in
    Error_handling.raise_if_error ();
    new c_listpairquantityquantity ptr
