...
class c_pairinstrumentportporttransform (h : unit ptr) : c_pairinstrumentportporttransform_t = object(self)
  val raw_val = h
  method raw = raw_val
...
module PairInstrumentPortPortTransform = struct
  ...
  let fromjson (json : string) : t =
    let ptr = Capi_bindings.pairinstrumentportporttransform_from_json_string (Falcon_string.of_string json) in
    Error_handling.raise_if_error ();
    new c_pairinstrumentportporttransform ptr
