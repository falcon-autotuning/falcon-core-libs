...
class c_listpairstringdouble (h : unit ptr) : c_listpairstringdouble_t = object(self)
  val raw_val = h
  method raw = raw_val
...
module ListPairStringDouble = struct
  ...
  let fromjson (json : string) : t =
    let ptr = Capi_bindings.listpairstringdouble_from_json_string (Falcon_string.of_string json) in
    Error_handling.raise_if_error ();
    new c_listpairstringdouble ptr
