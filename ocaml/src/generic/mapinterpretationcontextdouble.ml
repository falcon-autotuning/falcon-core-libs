...
class c_mapinterpretationcontextdouble (h : unit ptr) : c_mapinterpretationcontextdouble_t = object(self)
  val raw_val = h
  method raw = raw_val
...
module MapInterpretationContextDouble = struct
  ...
  let fromjson (json : string) : t =
    let ptr = Capi_bindings.mapinterpretationcontextdouble_from_json_string (Falcon_string.of_string json) in
    Error_handling.raise_if_error ();
    new c_mapinterpretationcontextdouble ptr
