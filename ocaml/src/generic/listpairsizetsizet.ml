...
class c_listpairsizetsizet (h : unit ptr) : c_listpairsizetsizet_t = object(self)
  val raw_val = h
  method raw = raw_val
...
module ListPairSizeTSizeT = struct
  ...
  let fromjson (json : string) : t =
    let ptr = Capi_bindings.listpairsizetsizet_from_json_string (Falcon_string.of_string json) in
    Error_handling.raise_if_error ();
    new c_listpairsizetsizet ptr
