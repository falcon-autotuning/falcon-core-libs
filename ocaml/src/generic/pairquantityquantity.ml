...
class c_pairquantityquantity (h : unit ptr) : c_pairquantityquantity_t = object(self)
  val raw_val = h
  method raw = raw_val
...
module PairQuantityQuantity = struct
  ...
  let fromjson (json : string) : t =
    let ptr = Capi_bindings.pairquantityquantity_from_json_string (Falcon_string.of_string json) in
    Error_handling.raise_if_error ();
    new c_pairquantityquantity ptr
