...
class c_listdevicevoltagestate (h : unit ptr) : c_listdevicevoltagestate_t = object(self)
  val raw_val = h
  method raw = raw_val
...
module ListDeviceVoltageState = struct
  ...
  let fromjson (json : string) : t =
    let ptr = Capi_bindings.listdevicevoltagestate_from_json_string (Falcon_string.of_string json) in
    Error_handling.raise_if_error ();
    new c_listdevicevoltagestate ptr
