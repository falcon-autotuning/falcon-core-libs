...
class c_listpairinterpretationcontextquantity (h : unit ptr) : c_listpairinterpretationcontextquantity_t = object(self)
  val raw_val = h
  method raw = raw_val
...
module ListPairInterpretationContextQuantity = struct
  ...
  let fillValue (count : int) (value : Pairinterpretationcontextquantity.PairInterpretationContextQuantity.t) : t =
  ...
  let fromjson (json : string) : t =
    let ptr = Capi_bindings.listpairinterpretationcontextquantity_from_json_string (Falcon_string.of_string json) in
    Error_handling.raise_if_error ();
    new c_listpairinterpretationcontextquantity ptr

  let pushBack (handle : t) (value : Pairinterpretationcontextquantity.PairInterpretationContextQuantity.t) : unit =
  ...
  let at (handle : t) (idx : int) : Pairinterpretationcontextquantity.PairInterpretationContextQuantity.t =
    Error_handling.read handle (fun () ->
      let result = Capi_bindings.listpairinterpretationcontextquantity_at handle#raw (Unsigned.Size_t.of_int idx) in
      Error_handling.raise_if_error ();
      new Pairinterpretationcontextquantity.c_pairinterpretationcontextquantity result
    )

  let items (handle : t) (out_buffer : Pairinterpretationcontextquantity.PairInterpretationContextQuantity.t) (buffer_size : int) : int =
  ...
  let contains (handle : t) (value : Pairinterpretationcontextquantity.PairInterpretationContextQuantity.t) : bool =
    Error_handling.multi_read [handle; value] (fun () ->
  ...
  let index (handle : t) (value : Pairinterpretationcontextquantity.PairInterpretationContextQuantity.t) : int =
    Error_handling.multi_read [handle; value] (fun () ->
