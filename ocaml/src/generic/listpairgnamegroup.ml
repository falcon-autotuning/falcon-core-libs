...
class c_listpairgnamegroup (h : unit ptr) : c_listpairgnamegroup_t = object(self)
  val raw_val = h
  method raw = raw_val
...
module ListPairGnameGroup = struct
  ...
  let fillValue (count : int) (value : Pairgnamegroup.PairGnameGroup.t) : t =
  ...
  let fromjson (json : string) : t =
    let ptr = Capi_bindings.listpairgnamegroup_from_json_string (Falcon_string.of_string json) in
    Error_handling.raise_if_error ();
    new c_listpairgnamegroup ptr

  let pushBack (handle : t) (value : Pairgnamegroup.PairGnameGroup.t) : unit =
  ...
  let at (handle : t) (idx : int) : Pairgnamegroup.PairGnameGroup.t =
    Error_handling.read handle (fun () ->
      let result = Capi_bindings.listpairgnamegroup_at handle#raw (Unsigned.Size_t.of_int idx) in
      Error_handling.raise_if_error ();
      new Pairgnamegroup.c_pairgnamegroup result
    )

  let items (handle : t) (out_buffer : Pairgnamegroup.PairGnameGroup.t) (buffer_size : int) : int =
  ...
  let contains (handle : t) (value : Pairgnamegroup.PairGnameGroup.t) : bool =
    Error_handling.multi_read [handle; value] (fun () ->
  ...
  let index (handle : t) (value : Pairgnamegroup.PairGnameGroup.t) : int =
    Error_handling.multi_read [handle; value] (fun () ->
