...
class OCamlGenerator:
    ...
    def resolve_type_path(self, c_type: str, current_class: ClassDef) -> str:
        ...
        if c_type.endswith("Handle"):
            ...
            if module_path:
                ...
                # Return just the type name with .t
                # The module will be opened via import
                return f"{type_name}.t"
            else:
                # Fallback
                return f"{type_name}.t"
    ...
    def c_to_ctypes(self, c_type: str) -> str:
        ...
        return self.primitive_map.get(c_type, "void")
    ...
    def generate_bindings(self):
        ...
        for cls in self.classes:
            ...
            for func in all_funcs:
                ...
                # FIXED: Proper parenthesization
                # Build argument chain with proper parentheses
                args_chain = ""
                for i, arg_type in enumerate(arg_ctypes):
                    if i == 0:
                        args_chain = f"({arg_type})"
                    else:
                        args_chain = f"{args_chain} @-> ({arg_type})"
...
"""
    ...
    def generate_wrappers(self):
        ...
        # Generate a main module that opens all submodules
        self.generate_main_module()

    def generate_single_module_file(self, cls: ClassDef):
        """Generate a single file for one module with watermark for imports"""
        # Generate .ml file (as before)
        ml_lines = []
        ml_lines.append("open Ctypes")
        ml_lines.append("open Capi_bindings")
        ml_lines.append("open ErrorHandling")
        ml_lines.append("")
        ...
        # Generate and write .mli file (interface)
    ...
    def generate_module_interface(self, cls: ClassDef) -> list[str]:
        ...
        # Add opens for dependencies
        dependencies = self.type_dependencies.get(cls.name, set())
        import_lines = []
        if dependencies:
            for dep in sorted(dependencies):
                parts = dep.split(".")
                if len(parts) > 1:
                    module_path = "Falcon_core." + ".".join(parts[:-1])
                    import_lines.append(f"open {module_path}")
        ...
        # CONCRETE class type (not polymorphic)
        ...
        # Module signature with CONCRETE types
    ...
    def inject_imports(self, file_path: Path, cls: ClassDef):
        """
        Inject imports at the watermark, similar to Go's insertImports.
        Like Go's genFile/main.go lines 73-95
        """
        # Read file
        content = file_path.read_text()
        lines = content.split("\n")

        # Find dependencies that were collected
        dependencies = self.type_dependencies.get(cls.name, set())

        # DEBUG: Print what we found
        if dependencies:
            print(f"  Dependencies for {cls.name}: {dependencies}")
        else:
            print(f"  No dependencies found for {cls.name}")

        # Convert to import paths
        import_lines = []
        if dependencies:
            for dep in sorted(dependencies):
                # dep is like "Physics.Device_structures.Connection"
                # We want: open Falcon_core.Physics.Device_structures
                parts = dep.split(".")
                if len(parts) > 1:
                    module_path = "Falcon_core." + ".".join(parts[:-1])
                    import_lines.append(f"open {module_path}")
                    print(f"    Adding import: {module_path}")
        ...
        for line in lines:
            if IMPORT_WATERMARK in line:
                # Replace watermark with imports
                if import_lines:
                    out.extend(import_lines)
                else:
                    out.append("(* no extra imports *)")
            else:
                out.append(line)
    ...
    def generate_main_module(self):
        """Generate the main Falcon_core module that aggregates everything"""
        lines = [
            "(* Auto-generated main module *)",
            "open Ctypes",
            "open Capi_bindings",
            "",
            self.generate_error_handling_module(),
            "",
        ...
        namespace_groups = {}
        for cls in self.classes:
            ns_key = tuple(cls.namespace_path) if cls.namespace_path else ()
            if ns_key not in namespace_groups:
                namespace_groups[ns_key] = []
            namespace_groups[ns_key].append(cls)

        # Generate nested module structure with includes
        for ns_path, classes in sorted(namespace_groups.items()):
            # Open nested modules
            for part in ns_path:
                lines.append(f"  module {part.capitalize()} = struct")

            # Include each class module
            for cls in classes:
                module_file = "/".join(
                    [p.lower() for p in ns_path] + [cls.name.lower()]
                )
                lines.append(f"    include {cls.name}")
